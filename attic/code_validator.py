# FROM SWEEPAI'S LATEST COMMIT. SAME AS UTILS.PY OF PREVIOUS VERSION
...
def get_new_lint_errors_for_pylint(new_errors: str, old_errors: str) -> str:
    ...
    try:
        for line in additional_errors:
            ...
            if error_type.startswith("E") or old_error_types.count(
                    error_type) < 2:  # if there are more than 1 of the same error, we consider it new
                results.append(line)
...
def get_new_lint_errors_for_eslint(new_errors: str, old_errors: str) -> str:
    ...
    for line in additional_errors:
        *_, error_type = line.split(" ")
        if not line.startswith("✖") and old_error_types.count(
                error_type) < 2:  # if there are more than 1 of the same error, we consider it new
            results.append(line)
    return "\n".join(results)
